  ...
  const selectElement = (selectElement, setIndex = 0) => {
    if (selectElement) {
      [].forEach.call(getAllElements(), (element, index) => {
        ...
        element.setAttribute("nav-hidden", 
            index < parseInt(selectElement.getAttribute("nav-index"))-4 ||
            (index > parseInt(selectElement.getAttribute("nav-index"))+3 && index > 8));
        if (selectThisElement) {
          //selectThisElement.scrollIntoView(true);
          console.log("scroll to:");
          //console.log(selectThisElement.getAttribute("nav-selected"));
          if(selectThisElement.getAttribute("nav-selected")==true){
            //selectThisElement.scrollIntoView(true);
